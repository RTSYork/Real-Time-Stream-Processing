...
//    }
	...
	static final class DefaultForkJoinWorkerThreadFactory
		...
		public final ForkJoinWorkerThread newThread(ForkJoinPool pool) {
			...
			try{
				fwt.setPriority(5); //Java priority
			}catch(Exception e){
    ...
    static final class WorkQueue {
        ...
        final void pushWithoutNotify(ForkJoinTask<?> task) {
            ...
            if ((a = array) != null) {    // ignore if queue removed
				...
				if (pool instanceof RealtimeForkJoinPool) {
					((RealtimeForkJoinPool) pool).increaseJobsleftCount(1);
					if (this.pool.StaticAllocationEnabled) {
						itemArrivalId++;
						SPRY.DataAllocation.DataAllocationPolicy DAP = ((RealtimeForkJoinPool) pool).getAllocationPolicy();
						int index = DAP.get(itemArrivalId); //get which Core shall it go to
						Thread wr = pool.workers.get(index);
						if (pool.allocations.get(wr) == null)
							pool.allocations.put(wr, new ArrayList<>(4096));
						int taskLocationInQueue = base + itemArrivalId - 1;
						pool.allocations.get(wr).add(taskLocationInQueue); // put the location of target item into its allocations
					}
				}
...
//        	if(pool.DEV_StaticAllocationEnabled){
//        		System.out.println("***Static task allocation Info for "+workPushedCount+" tasks, execpet task 0");
//        		pool.allocations.entrySet().forEach(e->{
//        			System.out.print(Affinity.get(e.getKey()).getProcessors()+":\t");
//					e.getValue().forEach(x -> System.out.print(x - base + 1 + ","));
//        			System.out.println();
//        		});
//        	}
        	/* workers are already sorted by CPU_ID ASC according to the implementation. */
    ...
    public void createAllWorkerThreads(){
    	/* Initialise the workQueues HaiTao
...
//        ws=workQueues;
//        int m = ws.length - 1;
//        WorkQueue q; int k = r & m & SQMASK;
//        if (((ps = plock) & PL_LOCK) == 0) { // create new queue
//            q = new WorkQueue(this, null, SHARED_QUEUE, r);
//            q.poolIndex = (short)k;
//            if (((ps = plock) & PL_LOCK) != 0 ||
//                !U.compareAndSwapInt(this, PLOCK, ps, ps += PL_LOCK))
...
//            if ((ws = workQueues) != null && k < ws.length && ws[k] == null)
//                ws[k] = q;
//            int nps2 = (ps & SHUTDOWN) | ((ps + PL_LOCK) & ~SHUTDOWN);
//            if (!U.compareAndSwapInt(this, PLOCK, ps, nps2))
//                releasePlock(nps2);
//        }
//        /* ******************************************************/
...
//        }
    ...
    protected boolean StaticAllocationEnabled = false;
    ...
    protected final int scan(WorkQueue w, int r) {
		...
		for (; i < searchTimes; i++) {
			 ...
			 * only the thread on the smallest CPU_ID (i.e., the first CPU in the scheduling partition) to do this procedure */
...
//        }
    ...
    private int workOnLocalQueues(){
    	Thread wt = Thread.currentThread();    	
		if (StaticAllocationEnabled && targetQueue != null) {
		...
		if (targetQueue != null) if (targetQueue.base < targetQueue.top) {
			System.out.println("Local Queue:\t"+Affinity.get(Thread.currentThread()).getProcessors()+"\tAt\t"+System.currentTimeMillis());
		...
		///* brute force search to determine the target thread's task queue, has same functionality to the above one		
		if (targetQueue == null)	for (int i = 0; i < workQueues.length; i++) {			
			if (workQueues[i] != null) {
				if (workQueues[i].owner != null) {// get the local task queue
					if (workQueues[i].base < workQueues[i].top) {
						//System.out.println("Local Queue:\t" + workQueues[i].base + "-" + workQueues[i].top+"\tBy\t"+Affinity.get(Thread.currentThread()).getProcessors());
						if (workQueues[i].array != null) {
							synchronized (workQueues[i].array) {
								ForkJoinTask<?> t = workQueues[i].array[workQueues[i].base];
								if (t != null) {
									U.compareAndSwapObject(workQueues[i].array, workQueues[i].base, t, null);
									workQueues[i].base++;
									t.doExec();
									return 0;
								}
			...
			if (globalQueue != null) if (globalQueue.base < globalQueue.top) {
				if (globalQueue.array != null) {
					synchronized (globalQueue.array) {
						...
						if (t != null) {
							//System.out.println("Shared Queue Stealing By\t" + Affinity.get(Thread.currentThread()).getProcessors());
        ...
        for(int i=0;i<commons.length;i++){
        	if(i<1){ //commons[0] should be normal Java thread pool
	        	...
	        	commons[i]=new ForkJoinPool(Runtime.getRuntime().availableProcessors(),
	    				new RealtimeForkJoinWorkerThreadFactory(new PriorityParameters(RTSJminPriority+i-1),null),
	    				null,
	    				false);
        	}
        }
        //int par = common.parallelism; // report 1 even if threads disabled
        int par = Runtime.getRuntime().availableProcessors(); //HaiTao
        commonParallelism = par > 0 ? par : 1;
    }

    //HaiTao
    public static ForkJoinPool getCommonPool(int priority){
    	if(priority<RTSJminPriority || priority>RTSJmaxPriority){ //not a RTSJ priority level
    		return commons[0];
    	}
    	else{//RTSJ priority levels
			if(commons[priority-RTSJminPriority+1]==null){
				commons[priority-RTSJminPriority+1] = new ForkJoinPool(Runtime.getRuntime().availableProcessors(),
	    				new RealtimeForkJoinWorkerThreadFactory(new PriorityParameters(priority),null),
	    				null,
	    				false);
			}
