...
final class ForEachOps {
    ...
    static final class ForEachTask<S, T> extends CountedCompleter<Void> {
		...
		public void compute() {
            ...
            Sink<S> taskSink = sink;	// AbstractPipeline.wrapSink
            ForEachTask<S, T> task = this;
            
            rightSplit = spliterator;
            //System.out.println("Split\t"+Thread.currentThread().getId()+"\t"+System.currentTimeMillis());
			if (spliterator instanceof SPRY.DataAllocation.RealtimeSpliterator){
				targetSize = sizeThreshold = ((SPRY.DataAllocation.RealtimeSpliterator<S>)spliterator).getGranularity();
			}else{
				if (getPool() != null)
					targetSize = sizeThreshold = sizeEstimate/ (getPool().getParallelism()<< 2);
				rightSplit = spliterator = new SPRY.DataAllocation.RealtimeSpliterator<S>(spliterator, targetSize);
			}

			while (!isShortCircuit || !taskSink.cancellationRequested()) {
                if (sizeEstimate <= sizeThreshold ||
                    (leftSplit = rightSplit.trySplit()) == null) {
                    task.helper.copyInto(taskSink, rightSplit);//AbstractPipeline.copyInto
                    break;
                }                
                ...
                // HaiTao
                /* *************************************************************/
                /* right is the current. Split it fully to partitions.
                 * push all partitions into queue, but hold the first bit*/
				...
				for (int i = 0; i < partitions.size(); i++) {
					...
					//s.forEachRemaining(x->System.out.println("push-->"+x));
				}
				/* wake up the other workers */
	            //System.out.println("notify workers");
				task.notifyWorkers();
				/* *************************************************************/
				/*
                if (forkRight) {
            ...
            //System.out.print(Affinity.get(Thread.currentThread()).getProcessors()+"\t");
    ...
    static final class ForEachOrderedTask<S, T> extends CountedCompleter<Void> {
        ...
        private static <S, T> void doCompute(ForEachOrderedTask<S, T> task) {
            ...
            while (rightSplit.estimateSize() > sizeThreshold &&
                ...
                /* HaiTao
                 * Always submit the task with the current Spliterator into the queue.
                 * */
                //forkRight=true;System.out.println("ForEachOrderedTask blocking spliterator is submitted to the queue");
                if (forkRight) {
                    forkRight = false;
                    rightSplit = leftSplit;
                    task = leftChild;
