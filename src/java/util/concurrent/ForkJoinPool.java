...
import SPRY.RealtimeForkJoinPool.RealtimeForkJoinWorkerThreadFactory;
...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    /* change the default factory so that real-time worker threads will be created,
     * but in a normal Java priority, i.e., 5 */
	static final class DefaultForkJoinWorkerThreadFactory implements ForkJoinWorkerThreadFactory {
		public final ForkJoinWorkerThread newThread(ForkJoinPool pool) {
			ForkJoinWorkerThread fwt = new ForkJoinWorkerThread(pool);
			try {
				fwt.setPriority(5);
			} catch (Exception e) {
				fwt.setSchedulingParameters(new PriorityParameters(PriorityScheduler.instance().getMinPriority()));
			}
			return fwt;
		}
	}
    ...
    static final class WorkQueue {
        ...
        final void push(ForkJoinTask<?> task) {
        	/* increase the pending job account */
			if (pool instanceof RealtimeForkJoinPool) ((RealtimeForkJoinPool) pool).increaseJobsleftCount(1);
        ...
        /* New added function, which allows a ForkJoinTask can be pushed 
         * into a worker's queue according to a given data allocation policy,
         *  but without notifying the worker to start to work */
        final void pushWithoutNotify(ForkJoinTask<?> task) {
            ForkJoinTask<?>[] a;
            int s = top, n;
            if ((a = array) != null) {    // ignore if queue removed
                int m = a.length - 1;
                U.putOrderedObject(a, ((m & s) << ASHIFT) + ABASE, task);
				if (pool instanceof RealtimeForkJoinPool) {
					((RealtimeForkJoinPool) pool).increaseJobsleftCount(1);
					if (this.pool.PreAllocationEnabled) {
						SPRY.DataAllocation.DataAllocationPolicy DAP = ((RealtimeForkJoinPool) pool).getAllocationPolicy();
						/* get which Core shall it go to */
						int CPU_ID = DAP.get(pool.itemArrivalId);
						// System.out.println("Push to processor:" + CPU_ID);
						Thread wr = pool.getWorkerIndex(CPU_ID);
						if (pool.allocations.get(wr) == null) pool.allocations.put(wr, new ArrayList<>(4096));
						int taskLocationInQueue = base + pool.locationCounter;
						pool.locationCounter++;
						pool.allocations.get(wr).add(taskLocationInQueue);
						pool.itemArrivalId++;
					}
				}
				/* set the global shared queue */ 
				pool.targetQueue = this;
				if ((n = (top = s + 1) - base) <= 2) {
					// (p = pool).signalWork(p.workQueues, this); /* not notify */
				} else if (n >= m) growArray();
            }
        }

		/* wakes up all worker threads */
		public final void notifyWorkers() {
			/*
			 * workers are already sorted by CPU_ID ASC according to the
			 * implementation
			 */
			for (int j = 0; j < pool.workers.size(); j++) {
				U.unpark(pool.workers.get(j));
			}
		}
     ...
     */
    static ForkJoinPool[] commons;
    public static final int RTSJminPriority=PriorityScheduler.instance().getMinPriority();
	public static final int RTSJmaxPriority=PriorityScheduler.instance().getMaxPriority();
            ...
            if (U.compareAndSwapLong(this, CTL, c, nc)) {
                ...
                try {
                    if ((fac = factory) != null &&
                    	...
                    	if(submitterThread==null)	submitterThread = wt;
                        wt.start();
                        break;
                    }
                } catch (Throwable rex) {
    ...
    final void externalPush(ForkJoinTask<?> task) {
    	/* identifying the prologue thread */
    	BitSet affinityS = Affinity.get(Thread.currentThread()).getProcessors();
		for (int i = 0; i < workers.size(); i++) {			
			BitSet w = Affinity.get(workers.get(i)).getProcessors();
			if (w.equals(affinityS)) submitterThread = workers.get(i);
		}
		if (submitterThread == null) {
			submitterThread = workers.get(0);
		}
    	
        if(this instanceof RealtimeForkJoinPool)
        	((RealtimeForkJoinPool)this).increaseJobsleftCount(1);
    ...
    /**
     * Pre-populate all the worker threads in the pool
     */
    public void createAllWorkerThreads(){
    	/* Initialise the workQueues
    	 * otherwise, worker threads can not be created correctly */
    	WorkQueue[] ws;  int ps;
    	int p = parallelism;            // find power of two table size
        int n = (p > 1) ? p - 1 : 1;    // ensure at least 2 slots
        n |= n >>> 1; n |= n >>> 2;  n |= n >>> 4;
		...
		for (int j = 0; j < workQueues.length; j++) {
			if (workQueues[j] != null) {
				if (workQueues[j].owner != null) {
					if (!workers.contains(workQueues[j].owner)) {
						workers.add(workQueues[j].owner);
						/* record affinity */
						BitSet a = Affinity.get(workQueues[j].owner).getProcessors();
						int processor = 0;
						for (; processor < a.size(); processor++)
							if (a.get(processor)) break;
						CPUToThread.put(processor, workQueues[j].owner);
					}
				}
			}
		}
     ...
     */
    /* Variables defined for the data allocation or accessing of the real-time pool */
    protected WorkQueue globalQueue = null;
    protected WorkQueue targetQueue = null;
    protected Thread submitterThread = null;
    protected ArrayList<Thread> workers = new ArrayList<>();
    protected boolean PreAllocationEnabled = false;
    protected HashMap<Thread, ArrayList<Integer>> allocations = new HashMap<>();
    protected final int scan(WorkQueue w, int r) {
    	int searchTimes = (workQueues.length) * 2 + 1;
		int i = 0;
		for (; i < searchTimes; i++) {
	    	/* take task from the target local queue */
			if (workOnLocalQueues() == 0) return 0;
			/* take task from the shared queue unless the local queue is empty
			 * only the thread on the smallest CPU_ID (i.e., the first CPU in the affinity) to do this procedure */
			if (workOnSharedQueue() == 0) return 0;
		}
    	/* park a worker if no work to do */
		if (i >= searchTimes) {
    ...
    /* Worker thread working on its allocations, i.e., taking ForkJoinTasks from its local queue */
    private int workOnLocalQueues(){
		Thread wt = Thread.currentThread();
		/* get the data from the queue that stores it.
		 * If pre-allocation is enabled, take according to the allocation policy.
		 * Otherwise, take from the head of the queue */
		if (PreAllocationEnabled && targetQueue != null) {
			ArrayList<Integer> allocated = allocations.get(wt);
			if (allocated == null) return -1;
			while (allocated.size() > 0) {
				int taskLocation = allocated.remove(0);
				ForkJoinTask<?> t = targetQueue.array[taskLocation];
				if (t != null) {
					U.compareAndSwapObject(targetQueue.array, taskLocation, t, null);
					t.doExec();
					if (this instanceof RealtimeForkJoinPool) ((RealtimeForkJoinPool) this).increaseJobsleftCount(-1);
					return 0;
				}
			}
			return -1;
		}
		else if (targetQueue != null) if (targetQueue.base < targetQueue.top) {			
			if (targetQueue.array != null) {
				synchronized (targetQueue.array) {
					ForkJoinTask<?> t = targetQueue.array[targetQueue.base];
					if (t != null) {
						U.compareAndSwapObject(targetQueue.array, targetQueue.base, t, null);
						targetQueue.base++;
						t.doExec();
    ...
    /* Taking task from a shared global queue */
    private int workOnSharedQueue(){
		if (submitterThread.equals(Thread.currentThread())) {
			if (globalQueue == null) globalQueue = getGlobalQueue();
			if (globalQueue != null) if (globalQueue.base < globalQueue.top) {
				if (globalQueue.array != null) {
					synchronized (globalQueue.array) {
						ForkJoinTask<?> t = globalQueue.array[globalQueue.base];
						if (t != null) {
							U.compareAndSwapObject(globalQueue.array, globalQueue.base, t, null);
							globalQueue.base++;
							t.doExec();
							if (this instanceof RealtimeForkJoinPool) ((RealtimeForkJoinPool) this).increaseJobsleftCount(-1);
							return 0;
	...
	private WorkQueue getGlobalQueue() {
		for (int i = 0; i < workQueues.length; i++) {
			if (workQueues[i] != null) {
				if (workQueues[i].owner == null) { return workQueues[i]; }
			}
		}
		return null;
	}
	
	/* Record the item arrival index */
	private int itemArrivalId = 0;
	private int locationCounter = 0;
	public void resetItemArrivalCounter() {
		itemArrivalId = 0;
	}	
	
	/* Get the worker thread from CPU index */
	HashMap<Integer,Thread> CPUToThread = new HashMap<>();
	public Thread getWorkerIndex(int CPU_ID) {
		return CPUToThread.get(CPU_ID);
	}
    /**
    ...
    private boolean tryTerminate(boolean now, boolean enable) {
		...
		for (int i = 0; i < commons.length; i++) {
			if (this == commons[i]) { return false; }
		}
...
//                (m = ws.length - 1) >= 0) ?
//            ws[m & z & SQMASK] : null;
    	/* Replace the orignal pool with the real-tiem pool */
		ForkJoinPool p;
		WorkQueue[] ws;
		int m, z;
		ForkJoinPool common;
		...
		return ((z = ThreadLocalRandom.getProbe()) != 0 && (p = common) != null && (ws = p.workQueues) != null && (m = ws.length - 1) >= 0) ? ws[m & z & SQMASK]
				: null;
    }
    ...
    public static ForkJoinPool commonPool() {
        ...
        //return common;
    	/* Return a common real-time pool */
		int prio = Thread.currentThread().getPriority();
		if (Thread.currentThread() instanceof RealtimeThread) {
			prio = ((PriorityParameters) RealtimeThread.currentRealtimeThread().getSchedulingParameters()).getPriority();
		}
		return getCommonPool(prio);
    }
...
//            return false;
//        }
        /* wait for all the common real-time pools to terminate */
		for (int i = 0; i < commons.length; i++) {
			if (this == commons[i]) {
				awaitQuiescence(timeout, unit);
				return false;
    ...
    static void quiesceCommonPool() {
    	...
    	/* Waits and/or attempts to assist performing tasks indefinitely
    	 * until all the common real-time pools */
    	for(int i=0;i<commons.length;i++){
        	commons[i].awaitQuiescence(Long.MAX_VALUE, TimeUnit.NANOSECONDS);
        }
    }
...
//                public ForkJoinPool run() { return makeCommonPool(); }});
        /* Make common real-time Pools*/
		commons = new ForkJoinPool[1 + (RTSJmaxPriority - RTSJminPriority + 1)];
		for (int i = 0; i < commons.length; i++) {
			if (i < 1) { /* commons[0] should be normal Java thread pool */
				commons[i] = makeCommonPool();
			} else {
				commons[i] = new ForkJoinPool(Runtime.getRuntime().availableProcessors(),
						new RealtimeForkJoinWorkerThreadFactory(new PriorityParameters(RTSJminPriority + i - 1)), null, false);
			}
		}
        /* common pool's parallelism */
        int par = Runtime.getRuntime().availableProcessors();
        commonParallelism = par > 0 ? par : 1;
    }

    /* get a real-time common pool */
    public static ForkJoinPool getCommonPool(int priority){
    	if(priority<RTSJminPriority || priority>RTSJmaxPriority){
			/* not a RTSJ priority level */
    		return commons[0];
    	}
		else {
			if (commons[priority - RTSJminPriority + 1] == null) {
				commons[priority - RTSJminPriority + 1] = new ForkJoinPool(Runtime.getRuntime().availableProcessors(),
						new RealtimeForkJoinWorkerThreadFactory(new PriorityParameters(priority)), null, false);
			}
			return commons[priority - RTSJminPriority + 1];
    	}
    }
	
    public static ForkJoinPool[] getCommonPools(){
    	return commons;
    }
