...
public class RealtimeForkJoinWorkerThreadFactory implements ForkJoinWorkerThreadFactory {
	...
	public RealtimeForkJoinWorkerThreadFactory(PriorityParameters priority, SchedulingPolicy schdlPolicy, BitSet affinity,
			ProcessingGroup... server) {
		this(priority, schdlPolicy, affinity, -1, server);
	}

	public RealtimeForkJoinWorkerThreadFactory(PriorityParameters priority, SchedulingPolicy schdlPolicy, BitSet affinity,
			int PrologueProcessor, ProcessingGroup... servers) {
	...
	public ForkJoinWorkerThread newThread(ForkJoinPool pool) {
		if (servers != null) {
			...
			if (servers.length > 0) {
				/*
				 * nextAvailableProcessorId is incremented after
				 * getNextAffinity() is invoked
				 */
				return new RealtimeForkJoinWorkerThread(pool, priority, servers[(nextAvailableProcessorId % servers.length)],
						getNextAffinity());
			}
		}
	...
	private void getAvailableProcessors(BitSet affinity, int PrologueProcessor) {
		...
		/* init array */
		//System.out.println("Totoal Number of Processors in Pool: " + numberOfProcessors);
